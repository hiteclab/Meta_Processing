...
class PWindow3 extends PApplet {
  ...
  void settings() {
    ...
    if (idlee == 10 || idlee == -10) {
       pin = objetoactual.getInt("pin");
       falsetrue = boolean(objetoactual.getInt("valor"));
    }
    nargu = nargumentos;
    lastid =idlee;
  ...
  void mousePressed() {
    ...
    if (condicion==true) { 
      ...
      if (posy== 120 && mouseX>352 && mouseX<368) {
        ...
        mousebif = true;
        keyif = false;
        if (mouseButton == LEFT) valor2 = "37";
        if (mouseButton == CENTER) valor2 = "3";
        if (mouseButton == RIGHT) valor2 = "39";
      }
    ...
    if (lastid == 10 || lastid == 11 || lastid == 12 || lastid == 13 || lastid == -10 || lastid == -11 || lastid == -12 || lastid == -13) {  // ARDUINO
      ...
      if (lastid==10 && posy==40 || lastid==-10 && posy==40) { // seleccionar casilla salidadigital
        ...
        if ( lastid==11 && posy==40 || lastid==13 && posy==40 || lastid==-11 && posy==40|| lastid==-13 && posy==40) { // seleccionar casilla salidadigital
          if (x==100) { 
            pin =0;
          } // pin 0
          if (x==120) { 
  ...
  void splash() {
    surface.setTitle("META_PROCESSING Version Alpha 1.2b");
    ...
    fill(255);
    text("META_PROCESSING Version Alpha 1.2b", 255, 340);
    fill(120);
  ...
  void config() {
    ...
    String[] ver = loadStrings(codefolder+"/version.txt");
    text(idiomagui.getString("Configuración")+": Meta_Processing "+ver[0], 10, 20); //else text(idiomagui.getString("Variable"), 10, 20);
    ...
    text(idiomagui.getString("Ultima version")+" Meta_Processing: "+newver[0], 50, 127);
    noStroke();
    fill(20);
    rect(41, 140, 90, 20);
    if (posy== 140 && mouseX>41 && mouseX<131) {
  ...
  void proyecto() {
    ...
    if (mousePressed== true) {
      ...
      if (posy== 80 && mouseX>170 && mouseX<260) {        
        ...
        global [0] = "// Generated with Meta_Processing Alpha 1.2b\n// https://github.com/hiteclab/Meta_Processing/releases\n";
